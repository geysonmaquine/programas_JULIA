...
function prob_media_retorno_quantico(t, base, autovalores, autovetores)
    N = length(autovalores)
    W = (base' * autovetores) .^ 2
    
    exp_values = [exp.(-im .* autovalores .* ti) for ti in t]
    pi = [mean(abs2.(W * exp_val)) for exp_val in exp_values]
...
function prob_transicao_quantica(t, base, autovalores, autovetores)
    N = length(autovalores)
    W = base' * autovetores
    exp_values = exp.(-im .* autovalores .* t)
    PI = abs2.(W * Diagonal(exp_values) * W')
    return PI
end
#function prob_transicao_quantica(t,base,autovalores,autovetores)
#    N=length(autovalores)
...
function valor_med_transicao(base::Matrix, autovalores::Vector, autovetores::Matrix)
    N = length(autovalores)
    W = base' * autovetores
    
    delta_matrix = [delta(autovalores[n], autovalores[m]) for n in 1:N, m in 1:N]
    
    X = zeros(N, N)
    for a in 1:N, b in 1:N
        X[a,b] = sum(delta_matrix .* (W[b,:] .* W[a,:])' * (W[a,:] .* W[b,:]))
    end
